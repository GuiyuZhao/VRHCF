...
class LearnableLogOptimalTransport(nn.Module):
    ...
    def forward(self, scores, row_masks=None, col_masks=None):
        ...
        if row_masks is None:
            row_masks = torch.ones(size=(batch_size, num_row), dtype=torch.bool)
        if col_masks is None:
            col_masks = torch.ones(size=(batch_size, num_col), dtype=torch.bool)

        padded_row_masks = torch.zeros(size=(batch_size, num_row + 1), dtype=torch.bool)
        padded_row_masks[:, :num_row] = ~row_masks
        padded_col_masks = torch.zeros(size=(batch_size, num_col + 1), dtype=torch.bool)
